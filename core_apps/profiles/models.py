...
class Profile(TimeStampedUUIDModel):
    ...
    def __str__(self):
        return f"{self.user.username}'s profile"  # type: ignore

    def following_list(self):
        return self.follows.all()

    def followers_list(self):
        return self.followed_by.all()  # type: ignore
    ...
    def check_is_followed_by(self, profile):
        return self.followed_by.filter(pkid=profile.pkid).exists()  # type: ignore
